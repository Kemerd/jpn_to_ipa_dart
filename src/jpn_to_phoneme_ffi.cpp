    ...
    std::string convert_with_segmentation(PhonemeConverter& converter, const std::string& japanese_text, WordSegmenter& segmenter) {
        ...
        for (size_t i = 0; i < words.size(); i++) {
            ...
            if (words[i].is_furigana_reading) {
                ...
                result += phoneme;
                if (g_logger_ptr) {
                    g_logger_ptr->log("  Converted word[" + std::to_string(i) + "] \"" + words[i].text + "\" → \"" + phoneme + "\" (from furigana)");
                }
            }
            // Special handling for the topic particle は → "wa"
            // Only apply this if NOT from furigana (furigana は is always "ha")
            ...
            // へそ (first word) → "heso", but 学校へ (second word) → "gakkou e"
                ...
                result += phoneme;
                if (g_logger_ptr) {
                    g_logger_ptr->log("  Converted word[" + std::to_string(i) + "] \"" + words[i].text + "\" → \"" + phoneme + "\"");
                }
            }
        }
